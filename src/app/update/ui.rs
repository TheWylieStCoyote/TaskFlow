...
fn handle_ui_work_log(model: &mut Model, msg: UiMessage) {
    ...
    match msg {
        UiMessage::ShowWorkLog => {
            // Only show if a task is selected
            if model.selected_index < model.visible_tasks.len() {
                model.show_work_log = true;
                model.work_log_selected = 0;
                model.work_log_mode = WorkLogMode::Browse;
                model.work_log_buffer = vec![String::new()];
                model.work_log_cursor_line = 0;
                model.work_log_cursor_col = 0;
            }
        }
        UiMessage::HideWorkLog => {
            model.show_work_log = false;
            model.work_log_mode = WorkLogMode::Browse;
            model.work_log_buffer = vec![String::new()];
            model.work_log_cursor_line = 0;
            model.work_log_cursor_col = 0;
        ...
        UiMessage::WorkLogUp => {
            if model.work_log_selected > 0 {
                model.work_log_selected -= 1;
            }
        }
        UiMessage::WorkLogDown => {
            if let Some(task_id) = model.visible_tasks.get(model.selected_index) {
                let entries = model.work_logs_for_task(task_id);
                if model.work_log_selected < entries.len().saturating_sub(1) {
                    model.work_log_selected += 1;
                }
            }
        }
        UiMessage::WorkLogView => {
            if model.work_log_mode == WorkLogMode::Browse {
                if let Some(task_id) = model.visible_tasks.get(model.selected_index) {
                    let entries = model.work_logs_for_task(task_id);
                    if entries.get(model.work_log_selected).is_some() {
                        model.work_log_mode = WorkLogMode::View;
                    }
                }
            } else if model.work_log_mode == WorkLogMode::View {
                // Return to browse mode when viewing and pressing Enter again
                model.work_log_mode = WorkLogMode::Browse;
            }
        }
        UiMessage::WorkLogAdd => {
            model.work_log_mode = WorkLogMode::Add;
            model.work_log_buffer = vec![String::new()];
            model.work_log_cursor_line = 0;
            model.work_log_cursor_col = 0;
        }
        UiMessage::WorkLogEdit => {
            if let Some(task_id) = model.visible_tasks.get(model.selected_index) {
                ...
                let content = entries
                    .get(model.work_log_selected)
                    .map(|e| e.content.clone());
                if let Some(content) = content {
                    model.work_log_mode = WorkLogMode::Edit;
                    // Load content into buffer as lines
                    model.work_log_buffer = content.lines().map(String::from).collect();
                    if model.work_log_buffer.is_empty() {
                        model.work_log_buffer.push(String::new());
                    }
                    model.work_log_cursor_line = 0;
                    model.work_log_cursor_col = 0;
                }
        ...
        UiMessage::WorkLogConfirmDelete => {
            if model.work_log_mode == WorkLogMode::Browse
                || model.work_log_mode == WorkLogMode::View
            {
                model.work_log_mode = WorkLogMode::ConfirmDelete;
            }
        }
        UiMessage::WorkLogCancel => {
        ...
        UiMessage::WorkLogSubmit => {
            if let Some(task_id) = model.visible_tasks.get(model.selected_index).cloned() {
                let content = model.work_log_buffer.join("\n");
        ...
        UiMessage::WorkLogDelete => {
            if model.work_log_mode == WorkLogMode::ConfirmDelete {
                if let Some(task_id) = model.visible_tasks.get(model.selected_index).cloned() {
                    let entries = model.work_logs_for_task(&task_id);
                    if let Some(entry) = entries.get(model.work_log_selected) {
                        let entry_id = entry.id.clone();
                        ...
                        if let Some(removed) = model.work_logs.remove(&entry_id) {
                            ...
                            if model.work_log_selected >= remaining.len() && !remaining.is_empty() {
                                model.work_log_selected = remaining.len() - 1;
                            }
        ...
        UiMessage::WorkLogInputChar(c) => {
            if matches!(model.work_log_mode, WorkLogMode::Add | WorkLogMode::Edit) {
                ...
                let line_idx = model
                    .work_log_cursor_line
                    .min(model.work_log_buffer.len() - 1);
                let col = model
                    .work_log_cursor_col
                    .min(model.work_log_buffer[line_idx].len());

                model.work_log_buffer[line_idx].insert(col, c);
                model.work_log_cursor_col = col + 1;
            }
        }
        UiMessage::WorkLogInputBackspace => {
            if matches!(model.work_log_mode, WorkLogMode::Add | WorkLogMode::Edit) {
                ...
                if line_idx < model.work_log_buffer.len() {
                    if model.work_log_cursor_col > 0 {
                        // Delete character before cursor
                        let col = model
                            .work_log_cursor_col
                            .min(model.work_log_buffer[line_idx].len());
                        if col > 0 {
                            model.work_log_buffer[line_idx].remove(col - 1);
                            model.work_log_cursor_col = col - 1;
                        }
                    } else if line_idx > 0 {
                        // At beginning of line - join with previous line
                        let current_line = model.work_log_buffer.remove(line_idx);
                        model.work_log_cursor_line = line_idx - 1;
                        model.work_log_cursor_col = model.work_log_buffer[line_idx - 1].len();
                        model.work_log_buffer[line_idx - 1].push_str(&current_line);
                    }
                }
            }
        }
        UiMessage::WorkLogInputDelete => {
            if matches!(model.work_log_mode, WorkLogMode::Add | WorkLogMode::Edit) {
                let line_idx = model.work_log_cursor_line;
                if line_idx < model.work_log_buffer.len() {
                    let line_len = model.work_log_buffer[line_idx].len();
                    if model.work_log_cursor_col < line_len {
                        // Delete character at cursor
                        model.work_log_buffer[line_idx].remove(model.work_log_cursor_col);
                    } else if line_idx + 1 < model.work_log_buffer.len() {
                        // At end of line - join with next line
                        let next_line = model.work_log_buffer.remove(line_idx + 1);
                        model.work_log_buffer[line_idx].push_str(&next_line);
                    }
                }
            }
        }
        UiMessage::WorkLogCursorLeft => {
            if matches!(model.work_log_mode, WorkLogMode::Add | WorkLogMode::Edit) {
                if model.work_log_cursor_col > 0 {
                    model.work_log_cursor_col -= 1;
                } else if model.work_log_cursor_line > 0 {
                    // Move to end of previous line
                    model.work_log_cursor_line -= 1;
                    model.work_log_cursor_col =
                        model.work_log_buffer[model.work_log_cursor_line].len();
                }
            }
        }
        UiMessage::WorkLogCursorRight => {
            ...
            if matches!(model.work_log_mode, WorkLogMode::Add | WorkLogMode::Edit) {
                ...
                if line_idx < model.work_log_buffer.len() {
                    ...
                    let col = model
                        .work_log_cursor_col
                        .min(model.work_log_buffer[line_idx].len());
                    let remainder = model.work_log_buffer[line_idx].split_off(col);
                    model.work_log_buffer.insert(line_idx + 1, remainder);
                    model.work_log_cursor_line += 1;
                    model.work_log_cursor_col = 0;
                }
            }
