//! TimeEntryRepository blanket implementation for in-memory backends.
...
impl<B: InMemoryBackend> TimeEntryRepository for B {
    fn create_time_entry(&mut self, entry: &TimeEntry) -> StorageResult<()> {
        ...
        self.data_mut().time_entries.push(entry.clone());
        self.mark_dirty();
        Ok(())
    }
    ...
    fn delete_time_entry(&mut self, id: &TimeEntryId) -> StorageResult<()> {
        ...
        self.data_mut().time_entries.retain(|e| &e.id != id);
