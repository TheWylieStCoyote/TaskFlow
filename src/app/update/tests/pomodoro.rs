...
fn test_pomodoro_skip_phase() {
    ...
    assert_eq!(model.pomodoro.session.as_ref().unwrap().cycles_completed, 1);
}
...
fn test_pomodoro_config_changes() {
    ...
    assert_eq!(model.pomodoro.config.work_duration_mins, 25);
    ...
    assert_eq!(model.pomodoro.config.cycles_before_long_break, 3);
}
