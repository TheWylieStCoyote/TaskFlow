//! HabitRepository blanket implementation for in-memory backends.
...
impl<B: InMemoryBackend> HabitRepository for B {
    fn create_habit(&mut self, habit: &Habit) -> StorageResult<()> {
        ...
        self.data_mut().habits.push(habit.clone());
        self.mark_dirty();
        Ok(())
    }
    ...
    fn update_habit(&mut self, habit: &Habit) -> StorageResult<()> {
        if let Some(existing) = self.data_mut().habits.iter_mut().find(|h| h.id == habit.id) {
    ...
    fn delete_habit(&mut self, id: &HabitId) -> StorageResult<()> {
        ...
        self.data_mut().habits.retain(|h| &h.id != id);
